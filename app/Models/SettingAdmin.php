...
class SettingAdmin extends Model
{
  ...
  public function calculatePrice($place, $checkin, $checkout, $numOfadults){
    ...
    $AllPrices = SettingAdmin::orderBy('id')->get();
     ...
     // checking for days in between 
     if ($checkin1 >= strtotime("2022-06-11") && $checkin1 <= strtotime("2022-06-11") && $checkout2 <= strtotime("2022-06-12")) {
      if (SettingAdmin::checkWeekOrNot($checkin) == 0) {
        if ($numOfadults == 1) {
          $price_tm += $AllPrices[0]->adult1_price + $AllPrices[1]->adult1_price;
        } else if ($numOfadults == 2) {
          $price_tm += $AllPrices[0]->adult2_price + $AllPrices[1]->adult2_price;
        } else if ($numOfadults == 3) {
          $price_tm += $AllPrices[0]->adult3_price + $AllPrices[1]->adult3_price;
        } else if ($numOfadults == 4) {
          $price_tm += $AllPrices[0]->adult4_price + $AllPrices[1]->adult4_price;
        }
      } else {
        if ($numOfadults == 1) {
          $price_tm += $AllPrices[0]->adult1_price_weekend + $AllPrices[1]->adult1_price_weekend;
        } else if ($numOfadults == 2) {
          $price_tm += $AllPrices[0]->adult2_price_weekend + $AllPrices[1]->adult2_price_weekend;
        } else if ($numOfadults == 3) {
          $price_tm += $AllPrices[0]->adult3_price_weekend + $AllPrices[1]->adult3_price_weekend;
        } else if ($numOfadults == 4) {
          $price_tm += $AllPrices[0]->adult4_price_weekend + $AllPrices[1]->adult4_price_weekend;
        }
      }
    }
